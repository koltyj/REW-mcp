...
- `rew-mcp` starts the stdio MCP server.
- `rew-mcp-tui` starts the optional terminal UI for the WebSocket bridge.

## Available Tools

### Offline Analysis & Interpretation

| Tool | Description |
|------|-------------|
| `rew.ingest_measurement` | Parse and store REW text exports |
| `rew.compare_measurements` | Compare before/after or multi-position measurements |
| `rew.analyze_room_modes` | Detect peaks, nulls, and theoretical room modes |
| `rew.analyze_decay` | Analyze RT60, EDT, and ringing from impulse data |
| `rew.analyze_impulse` | Detect early reflections and estimate paths |
| `rew.interpret_with_glm_context` | Explain results through Genelec GLM capabilities and limits |
| `rew.average_measurements` | Build spatial averages across positions |
| `rew.compare_to_target` | Score a response against flat, REW, Harman, or custom curves |
| `rew.analyze_room` | Run a prioritized full-room analysis |
| `rew.optimize_room` | Track one-change-at-a-time optimization progress |

### REW API Control & Guided Workflows

| Tool | Description |
|------|-------------|
| `rew.api_connect` | Connect to the local REW API |
| `rew.api_list_measurements` | List measurements currently loaded in REW |
| `rew.api_get_measurement` | Fetch a measurement from REW by UUID |
| `rew.api_audio` | Inspect and configure REW audio devices |
| `rew.api_generator` | Control the REW signal generator |
| `rew.api_spl_meter` | Start, stop, and read the SPL meter |
| `rew.api_measure` | Trigger or inspect measurements |
| `rew.api_measure_workflow` | Orchestrate setup, level checks, and measurement runs |
| `rew.api_check_levels` | Verify mic input levels before measuring |
| `rew.api_calibrate_spl` | Guide monitor calibration to a target SPL |
| `rew.api_measurement_session` | Run a stateful L/R/Sub measurement session |
| `rew.api_measurement_commands` | Run per-measurement REW commands |
| `rew.api_measurement_eq` | Inspect or update REW EQ/filter state |
| `rew.api_import` | Import measurement data into REW |
| `rew.api_eq` | Manage REW global EQ defaults |
| `rew.api_groups` | Manage REW measurement groups |
| `rew.api_rta` | Control the real-time analyzer |
...
Active resources are listed dynamically for current sessions and stored measurements. Resource templates are:
...
| `history://{session_id}` | Session measurement history and summaries |
...
| [Examples](docs/examples.md) | Example measurement and analysis workflows |
